...
VEV — **on-air NDI source**. A news presenter drives a live webpage that is broadcast into
...
- **Settings + NDI name persist** via `ConfigStore` (atomic tmp+rename, defaults-on-corrupt) to
  `userData`. `app.setName('VEV')` fixes the userData folder across dev/packaged — don't change it
...
`VEV_SELFCHECK=1 [VEV_SELFCHECK_MODE=presenter | _TRAY=1 | _CRASH=1] npm start` and the
independent `node tools/ndi-probe.cjs VEV 8`. A capture/NDI change is not done until the probe
